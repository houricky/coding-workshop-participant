...
from postgres_service import create_budget, delete_budget, get_budget, list_budgets, update_budget
...
def handler(event=None, context=None):
    try:
        ...
        if path.startswith("/") and len(path) > 1:
            ...
            if method == "GET":
                return get_one(budget_id)
            if method == "PUT":
                return update(budget_id, body)
            if method == "DELETE":
                return remove(budget_id)
            return error_response(405, "method_not_allowed", f"{method} not allowed")
...
def list_all(query: dict):
    ...
    budgets = list_budgets(project_id)
...
def update(budget_id: str, body: dict):
